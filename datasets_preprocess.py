...
def HyperEvaluate(config):
    ...
    for i in tqdm(range(len(dataset))):
        try:
            other_lang_gold = dataset["translation"][i][ext_language]
            eng_gold = dataset["translation"][i]["en"]
...
if __name__ == "__main__":
    ...
    PARAM_GRID = list(
        product(
            ...
            ["de", "lt"],  # languages#[verbAtBeginning]
            [
                treeMirrorPre,
                treeMirrorPo,
                treeMirrorIn,
    ...
    if debug:
        h_param_list = [h_param_list[0]]
        print(h_param_list)
        HyperEvaluate(h_param_list[0])
    else:
        # run by submitit
        d = datetime.today()
        exp_dir = (
